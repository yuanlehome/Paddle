...
class GenericPlugin : public DynamicPluginTensorRT {
 ...
 public:
  ...
  size_t getSerializationSize() const TRT_NOEXCEPT override {
    size_t sum = 0;
    sum += SerializedSize(kernel_signature_.name);
    sum += SerializedSize(kernel_signature_.input_names.size());
    for (size_t i = 0; i < kernel_signature_.input_names.size(); i++) {
      sum += SerializedSize(kernel_signature_.input_names[i]);
    }
    sum += SerializedSize(kernel_signature_.attr_names.size());
    for (size_t i = 0; i < kernel_signature_.attr_names.size(); i++) {
      sum += SerializedSize(kernel_signature_.attr_names[i]);
    }
    sum += SerializedSize(kernel_signature_.output_names.size());
    for (size_t i = 0; i < kernel_signature_.output_names.size(); i++) {
      sum += SerializedSize(kernel_signature_.output_names[i]);
    }
